...
## 2026-09-09：RL/评估沿用 AReaL SFT 政策

本轮按用户明确要求变更 agent 提示词与自定义奖励规则：逐字使用清洗后 AReaL
10,649 条 SFT 样本的共同 system 消息；不再追加官方 policy 或 action_proposal
协议。未取得用户确认不拦截、不作为确定性/过程/LLM Judge 违规；其他政策规则保留，
并补齐 SFT 明确的旅行券不得用于任何订单更新规则。

完整来源、提取命令、哈希校验与旧实验迁移边界见 README 的统一政策章节。
下面各历史轮次中涉及确认标签硬拦截的描述不再代表当前行为。官方 Tau2 评分未修改。

本地验证：Transformers 4.57.1 / 5.10.4 各 **247 passed, 1 skipped**；
唯一跳过的是旧 `RUN_QWEN_INTEGRATION` 可选测试，本轮真实本地 Qwen tokenizer
回归已运行。Ruff、`git diff --check`、30 train / 20 test 拆分及 50 条动作原文
校验均通过。模拟生产 rollout 证明无确认标签的合法写操作到达工具后端且奖励为 1；
其他政策违规仍会 gate 奖励。未加载 4B 权重、进行 GPU 训练或调用付费 API。
